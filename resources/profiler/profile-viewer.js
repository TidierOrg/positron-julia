...
class ProfileViewer {
  constructor(element, data, selectorLabel) {
    ...
    __publicField(this, "canvasHeightCSS");
    __publicField(this, "canvasWidthCSS");
    __publicField(this, "hoverCanvas");
    __publicField(this, "hoverCanvasCtx");
    ...
    __publicField(this, "isWheeling", false);
    __publicField(this, "canWheelDown", true);
    __publicField(this, "scrollPosition", 0);
    __publicField(this, "resizeObserver");
    __publicField(this, "isResizing", false);
    __publicField(this, "isDocumentScrolling", false);
    __publicField(this, "isMouseMove", false);
    __publicField(this, "scale", window.devicePixelRatio);
    __publicField(this, "borderWidth", 2);
    __publicField(this, "padding", 2);
    __publicField(this, "fontConfig", "10px sans-serif");
    __publicField(this, "borderColor", "#fff");
    __publicField(this, "selectorLabelElement");
    __publicField(this, "selectorLabel", "Thread");
    __publicField(this, "boxHeight", 24);
    __publicField(this, "ctrlClickHandler");
    __publicField(this, "selectionHandler");
  ...
  getStyles() {
    ...
    this.boxHeight = Math.ceil(
      ((textMetrics.fontBoundingBoxDescent ?? textMetrics.actualBoundingBoxDescent) + (textMetrics.fontBoundingBoxAscent ?? textMetrics.actualBoundingBoxAscent) + 2 * this.borderWidth + 2 * this.padding) * this.scale
    );
    if (this.activeNode) {
      this.redraw();
    }
        ...
        window.requestAnimationFrame(() => {
          ...
          if (didDraw) {
            if (mouseX > this.canvasWidthCSS / 2) {
              this.tooltipElement.style.right = this.canvasWidthCSS - mouseX + 10 + "px";
            ...
            if (mouseY > this.canvasHeightCSS / 2) {
              this.tooltipElement.style.bottom = this.canvasHeightCSS - mouseY + 10 + "px";
  ...
  updateTooltip(node) {
    ...
    this.tooltip.percentage.innerText = (100 * node.count / this.data[this.currentSelection].count).toFixed();
    const flags = [];
    if (node.flags & 1) {
  ...
  drawHover(node, mouseX, mouseY) {
    let found = false;
    this.runOnNodeAtMousePosition(node, mouseX, mouseY, (node2) => {
      this.drawHoverNode(node2);
      found = true;
    });
    return found;
  }
  runOnNodeAtMousePosition(root, x, y, f) {
    if (x >= Math.floor(root.pos.x) && x <= Math.ceil(root.pos.x + root.pos.width) && y >= root.pos.y) {
